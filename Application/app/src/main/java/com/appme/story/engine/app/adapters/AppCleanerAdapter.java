...
import com.appme.story.engine.graphics.LinearColorBar;
...
public class AppCleanerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<AppCleanerItem> mItems = new ArrayList<>();
    private List<AppCleanerItem> mFilteredItems = new ArrayList<>();
    ...
    public static class HeaderViewHolder extends RecyclerView.ViewHolder {
        ...
        public HeaderViewHolder(View itemView) {
            ...
            mColorBar.setColors(itemView.getResources().getColor(R.color.app_list_system_memory),
                    itemView.getResources().getColor(R.color.app_list_cache_memory),
                    itemView.getResources().getColor(R.color.app_list_free_memory));
        ...
        public void updateStorageUsage(long totalMemory, long lowMemory, long medMemory,
            ...
            mFreeSizeText.setText(context.getString(R.string.app_list_header_memory, sizeStr));

            sizeStr = bidiFormatter.unicodeWrap(
                    Formatter.formatShortFileSize(context, medMemory));
            mCacheSizeText.setText(context.getString(R.string.app_list_header_memory, sizeStr));

            sizeStr = bidiFormatter.unicodeWrap(
                    Formatter.formatShortFileSize(context, highMemory));
            mSystemSizeText.setText(context.getString(R.string.app_list_header_memory, sizeStr));
        ...
        switch (viewType) {
            case VIEW_TYPE_HEADER:
                return new HeaderViewHolder(LayoutInflater.from(viewGroup.getContext()).inflate(
                        R.layout.app_cleaner_list_header, viewGroup, false));

            case VIEW_TYPE_ITEM:
                return new ItemViewHolder(LayoutInflater.from(viewGroup.getContext()).inflate(
                        R.layout.app_cleaner_list_item, viewGroup, false));
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        if (viewHolder instanceof ItemViewHolder) {
            AppCleanerItem item = mFilteredItems.get(position);
    ...
    public long getItemId(int i) {
        AppCleanerItem item = mFilteredItems.get(i);
    ...
    private void insertHeaderView(List<AppCleanerItem> items) {
    ...
    public void setItems(Context context, List<AppCleanerItem> items, SortBy sortBy, String filter) {
        mItems = items;
    ...
    public void sortAndFilter(Context context, final SortBy sortBy, String filter) {
        if (sortBy != mLastSortBy) {
            ...
            ArrayList<AppCleanerItem> items = new ArrayList<>(mItems);

            Collections.sort(items, new Comparator<AppCleanerItem>() {
                @Override
                public int compare(AppCleanerItem lhs, AppCleanerItem rhs) {
                    switch (sortBy) {
                        case APP_NAME:
        ...
        if (filter != null && !filter.equals("")) {
            List<AppCleanerItem> filteredItems = new ArrayList<>();
            ...
            for (AppCleanerItem item : mItems) {
