package com.appme.story.engine.app.commons.manager;
...
public class DataManager {
    ...
    public List<AppItem> getTargetAppTimeline(Context context, String target, int offset) {
        ...
        if (manager != null) {
            ...
            long[] range = MonitorUsageUtil.getTimeRange(SortEnum.getSortEnum(offset));
            UsageEvents events = manager.queryEvents(range[0], range[1]);
            UsageEvents.Event event = new UsageEvents.Event();

            AppItem item = new AppItem();
            item.mPackageName = target;
            item.mName = MonitorUsageUtil.parsePackageName(context.getPackageManager(), target);
            ...
            while (events.hasNextEvent()) {
                ...
                if (currentPackage.equals(target)) { // 本次交互开始
                    ...
                    if (prevEndEvent != null && start > 0) {
                        ...
                        if (item.mUsageTime > MonitorUsageConstants.USAGE_TIME_MIX) item.mCount++;
                        items.add(item.copy());
                        start = 0;
                        prevEndEvent = null;
                    }
    ...
    public List<AppItem> getApps(Context context, int sort, int offset) {
                ...
                if (!prevPackage.equals(eventPackage)) { // 包名有变化
                    if (startPoints.containsKey(prevPackage) && endPoints.containsKey(prevPackage)) {
                        ...
                        if (listItem != null) { // update list item info
                            ...
                            if (duration > MonitorUsageConstants.USAGE_TIME_MIX) {
                                listItem.mCount++;
                            }
                        }
        ...
        if (items.size() > 0) {
            ...
            for (AppItem item : items) {
                if (!MonitorUsageUtil.openable(packageManager, item.mPackageName)) {
                    continue;
                }
                if (hideSystem && MonitorUsageUtil.isSystemApp(packageManager, item.mPackageName)) {
                    continue;
                }
                if (hideUninstall && !MonitorUsageUtil.isInstalled(packageManager, item.mPackageName)) {
                    continue;
                }
                ...
                if (valid) {
                    String key = "u" + MonitorUsageUtil.getAppUid(packageManager, item.mPackageName);
                    if (mobileData.size() > 0 && mobileData.containsKey(key)) {
                        item.mMobile = mobileData.get(key);
                    }
                }
                item.mName = MonitorUsageUtil.parsePackageName(packageManager, item.mPackageName);
                newList.add(item);
            }
