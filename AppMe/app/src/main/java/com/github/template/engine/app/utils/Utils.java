...
import com.github.template.engine.app.preview.MimeTypes;
import com.github.template.engine.app.tasks.ShareTask;
...
//                                Toast.LENGTH_LONG).show();
    ...
    public static Intent createFileOpenIntent(File file)
    {
        ...
        intent.setDataAndType(Uri.fromFile(file), MimeTypes.getFileMimeType(file));
        return intent;
    }
    ...
    public static void shareFiles(Activity c, ArrayList<File> a, int fab_skin) {
        ...
        String mime = MimeTypes.getFileMimeType(a.get(0));
        if (a.size() > 1)
            for (File f : a) {
                if (!mime.equals(MimeTypes.getFileMimeType(f))) {
                    b = false;
                }
            }
